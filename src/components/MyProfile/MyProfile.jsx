...
export default function MyProfile() {
  ...
  const updateAddress = async () => {
    ...
    console.log("updAdObj: ", updatedAddressObj);
    try {
      setIsLoading(true);
      const res = await axios.put(
        `${ENVConfig.API_ServerURL}/auth/profile`,
    ...
    const onClick = async (data) => {
      try {
        ...
        const response = await axios.put(
          `${ENVConfig.API_ServerURL}/auth/profileimg`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
        ...
        showToast(
          `Image upload successful to: ${
            response.data.profileimg.optimzied ||
            response.data.profileimg.original
          }`,
          "success"
        );
